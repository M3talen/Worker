...
public class SQLHandler extends SQLiteOpenHelper {
    ...
    public List<DataRecord> getRecordsByType(String tType, String tSort, String tACC)
    {
       ...
       // Log.d("aa", tType);
        ...
        {       //  ORDER BY CAST(substr(DATE,7) AS INTEGER) ASC, CAST(substr(DATE,4,1) AS INTEGER) ASC, CAST(substr(DATE,1,1) AS INTEGER) ASC
    ...
    public List<DataRecord> getRecordsByMonthAndYear(String tDate, String tSort, String tACC)
    {
        List<DataRecord> Record = new LinkedList<DataRecord>();
        String query = "";
        if (new String(tSort).equals("ASC"))
        {
            query = "SELECT * FROM " + TABLE_RECORDS + " WHERE ACC = '" + tACC + "' AND DATE = '" + tDate + "' ORDER BY CAST(substr(DATE,7) AS INTEGER) ASC, CAST(substr(DATE,4,2) AS INTEGER) ASC";
        }
        if (new String(tSort).equals("DESC"))
        {
            query = "SELECT * FROM " + TABLE_RECORDS + " WHERE ACC = '" + tACC + "' AND DATE = '" + tDate + "' ORDER BY CAST(substr(DATE,7) AS INTEGER) DESC, CAST(substr(DATE,4,2) AS INTEGER) DESC";
        }
